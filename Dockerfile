...
CMD sh -c "envsubst '\$PORT' < /etc/nginx/sites-available/app.conf > /etc/nginx/sites-available/app.conf.tmp" && cp -f /etc/nginx/sites-available/app.conf.tmp /etc/nginx/sites-available/app.conf && /usr/bin/supervisord -c /etc/supervisor/conf.d/supervisord.conf
